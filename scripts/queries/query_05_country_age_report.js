  ...
  .aggregate(
    [
      ...
      },
      {
        $project: {
          rows: {
            $concatArrays: ["$detail", "$countrySubtotal", "$grandTotal"],
          },
        },
      },
      { $unwind: "$rows" },
...
console.log(
  `\n${genreName} ratings: Country -> Age group -> Country subtotal -> Grand total`,
);
console.table(
  rows.map((row) => ({
    level: row.level,
    country: row.country || "ALL COUNTRIES",
    ageGroup:
      row.ageGroup ||
      (row.level === "COUNTRY_SUBTOTAL" ? "SUBTOTAL" : "ALL AGE GROUPS"),
